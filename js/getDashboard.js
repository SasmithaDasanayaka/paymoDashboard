...
function getWorkedHours(dateRange) {
  $.ajax({
    ...
    success: function (result) {
      ...
      Object.values(response.client).forEach((element) => {
        ...
        const amount = (element.workedHours3Months * 90).toFixed(2);
      ...
      <tr>
        ...
        <td class="text-center">
         ${timeShare}% (${element.workedHours3Months} hours)
        </td>
        <td class="text-center">
         ${amount}€
        </td>
...
function getUserDetails(dateRange) {
  $.ajax({
    url: "php/users.php",
    type: "GET",
    data: {
...
function dateChangeOverview() {
  dashBoardLoader.style.visibility = "visible";
  const dateRange = document.getElementById("dateSelectOverview").value;
  getWorkedHours(dateRange);
}

function dateChangeEmployee() {
  employeeLoader.style.visibility = "visible";
  const dateRange = document.getElementById("dateSelectEmployee").value;
  getUserDetails(dateRange);
}

(function () {
  getProjectDetails();
  getUserDetails("3_months");
  getWorkedHours("3_months");
})();
